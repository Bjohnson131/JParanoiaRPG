...
public class PrivateMessagePane extends JPanel {
	...
	public PrivateMessagePane(ServerPlayer paramServerPlayer) {
		...
		this.displayArea.addFocusListener(new FocusAdapter() {
			public void focusGained(FocusEvent paramAnonymousFocusEvent) {
				if (!JPServer.optionsMenu.quickCharsheetMenuItem.isSelected()
					...
					JPServer.serverOptions.setQuickNamesToggle(true);
				} else {
					JPServer.charsheetPanel.playerComboBox.setSelectedItem(PrivateMessagePane.this.player);
					JPServer.serverOptions.setQuickNamesToggle(false);
				}
		...
		this.scrollPane = new JScrollPane(this.displayArea, ScrollPaneConstants.VERTICAL_SCROLLBAR_ALWAYS,
		...
		this.inputLine.addActionListener(paramAnonymousActionEvent -> {
			...
			if (!str.equals("")) {
				PrivateMessagePane.this.player
						.specificSend("200" + PrivateMessagePane.this.player.getID() + JPServer.myPlayer.getID() + str);
	...
	public void addMyMessage(String paramString) {
		try {
			...
			this.privateMessageDocument.insertString(this.privateMessageDocument.getLength(),
					"  " + JPServer.myPlayer.getName().substring(0, 1) + ": ", pmAttributes);
			...
			if (JPServer.serverOptions.isKeepLog()) {
				...
				if (JPServer.serverOptions.isHtmlLog()) {
					str = "<span class=\"pm\"><span class=\"player0\">(" + JPServer.myPlayer.getName()
							+ "</span> -> <span class=\"player" + this.player.PLAYER_NUMBER + "\">"
							+ this.player.toString() + ")</span>: " + paramString + "</span><br/>";
				} else {
 ...
 * server\PrivateMessagePane.class Java compiler version: 2 (46.0) JD-Core
 * Version: 0.7.1
 */
